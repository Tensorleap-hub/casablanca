...
class Synthesis(nn.Module):
    ...
    def forward(self, wa, alpha, feats):
        ...
        out_warp, out, skip_flow = self.to_flows[0](out, latent[:, 3], feats[0])
        skip = self.to_rgbs[0](out_warp)

        i = 3
        for conv1, conv2, to_rgb, to_flow, feat in zip(self.convs[(i-1)::2], self.convs[i::2], self.to_rgbs[i//2:],
                                                          self.to_flows[i//2:], feats[i//2:]):
            out = conv1(out, latent[:, i])
            out = conv2(out, latent[:, i + 1])
            out_warp, out, skip_flow = to_flow(out, latent[:, i + 2], feat, skip_flow)
            skip = to_rgb(out_warp, skip)
