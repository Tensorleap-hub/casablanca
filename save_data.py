...
parser.add_argument('--save_path', type=str, default="/Users/chenrothschild/Tensorleap/data/casablanca/", help='Target directory');
parser.add_argument('--user', type=str, default="USERNAME", help='Username');
...
parser.add_argument('--extract', dest='extract', action='store_true', help='Enable extract', default=True)
# parser.add_argument('--convert', dest='convert', action='store_true', help='Enable convert', default=True)
...
def concatenate_files_from_gcs(bucket_name, lines, save_path):
    storage_client = storage.Client()
    bucket = storage_client.bucket(bucket_name)

    for line in lines:
        infile, outfile, md5gt = line.split()

        # Download file contents without saving it locally
        blob = bucket.blob(infile)
        ...
        if md5 == md5gt:
            print('Checksum successful for %s.' % outfile)
        else:
            raise Warning('Checksum failed for %s.' % outfile)

        # Optionally, you can delete the input file from GCS
        blob.delete()
